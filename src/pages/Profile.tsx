...
export function Profile({ onOpenSettings, onOpenArchivedHabits, onLogout, onBack }: ProfileProps) {
  const stats = [
  ...
  const menuSections = [
    {
      ...
      items: [
        ...
        { icon: Archive, label: 'Archived Habits', onClick: onOpenArchivedHabits },
  ...
  return (
    <div className="min-h-screen bg-background pb-8">
        ...
        <div className="glass-card p-6 bg-gradient-to-br from-primary/10 to-accent/10">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h2 className="text-xl font-bold text-foreground">Jamie Doe</h2>
              ...
              <div className="flex items-center gap-1 mt-2">
                ...
                <span className="text-small text-orange-500 font-medium">23 day streak 🔥</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="px-4 mb-6">
        <h3 className="text-small text-muted-foreground uppercase tracking-wider mb-3">Your Stats</h3>
          ...
          {stats.map((stat) => (
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-body font-bold text-foreground">{stat.value}</p>
      ...
      {menuSections.map((section) => (
        <div key={section.title} className="px-4 mb-6">
          <h3 className="text-small text-muted-foreground uppercase tracking-wider mb-3">{section.title}</h3>
      ...
      <div className="px-4">
        <button
          onClick={onLogout}
          className="w-full glass-card flex items-center justify-center gap-3 p-4 hover:bg-red-500/10 transition-colors group"
        >
          <LogOut className="w-5 h-5 text-red-400 group-hover:text-red-500 transition-colors" />
          <span className="text-body text-red-400 group-hover:text-red-500 font-medium transition-colors">Sign Out</span>
        </button>
      </div>
    </div>
  );
